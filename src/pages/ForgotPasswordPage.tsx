...
const ForgotPasswordPage = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/auth/${token ? "reset-password" : "request-password-reset"}`, {
        method: "POST",
      ...
      if (token) setTimeout(() => navigate("/login"), 1200);
    } catch (err) {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-background p-4">
    <div className="w-full max-w-md rounded-xl bg-card p-8 shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {!token ? <div><Label htmlFor="reset-email">อีเมล</Label><Input id="reset-email" type="email" className="mt-1" placeholder="example@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required /></div> : <>
          <p className="text-sm text-muted-foreground">กรอกรหัสผ่านใหม่สำหรับบัญชีของคุณ</p>
        ...
        <Button type="submit" className="w-full" disabled={isSubmitting}>{isSubmitting ? "กำลังดำเนินการ..." : token ? "เปลี่ยนรหัสผ่าน" : "ส่งลิงก์ไปยังอีเมล"}</Button>
      </form>
